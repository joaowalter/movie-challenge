...
export class CsvLoaderService {
    ...
    private parseYear(yearInString: string): number | null {
        try {
            ...
            if (isNaN(year) || year < 1895 || year > new Date().getFullYear()) {
                console.warn(
                    `Ano inválido ${yearInString} o primeiro filme foi em 1895, e não pode ser maior que o ano atual.`
                );
                return null;
            }
    ...
    private parseData(singleData: CsvRowData, line: number): MovieToAdd | null {
